...
# 看板
    ...
    give @s oak_sign{display: {Name: '{"text": "右クリ脱出", "color": "blue"}'}, BlockEntityTag: {is_waxed: true, front_text: {color: "blue", has_glowing_text: true, messages: ['{"text": "右クリックで脱出", "bold": true, "clickEvent": {"action": "run_command", "value": "/execute if entity @s[nbt={SelectedItem:{"id":"minecraft:diamond"}}] run tp @s 2 24 0 "}}', '""', '""', '""']}}}